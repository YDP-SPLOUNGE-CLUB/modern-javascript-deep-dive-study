...
브라우저의 렌더링 엔진은 다음과 같은 과정을 통해 응답받은 HTML 문서를 파싱하여 브라우저가 이해할 수 있는 자료구조인 **DOM을 생성**한다.
...
렌더링 엔진은 서버로부터 응답된 HTML과 CSS를 파싱하여 각 DOM과 CSSOM를 생성한다.
...
app.js 의 파싱과 실행 이전까지는 DOM의 생성이 일시 중단된다.
